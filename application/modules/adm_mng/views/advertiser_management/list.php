...
<div class="row">
    <div class="draw-back">
        <img src="<?php echo base_url(); ?>temp/manager/images/loading.gif" alt="" height="100px" width="100px">
        Loading...
    </div>
    <div class="box col-md-12">
        ...
        <div class="box-content">
            ...
            <div class="formsearch">
                ...
                <div class="col-md-4">
                    <form action="<?= base_url('manager/advertiser/list_account') ?>" method="POST">
                        <div class="col-md-10"><input name="keycode" class="form-control" id="keys"
                                placeholder="ID or Email" value="<?= $this->session->userdata('keycode') ?>" /></div>
                        <div class="col-md-2 search"><button class="btn btn-default">Search</button></div>
                    </form>
                </div>

                <div class="col-md-2 row custom-filter">
                    <label class="col-md-4 label2" for="">Balance</label>
                    <div class="col-md-8">
                        <select class="form-control input-sm sorting" name="balance">
                            <option value="">Sorting...</option>
                            <option value="desc" <?= $sort == 'balance' && $order == 'desc' ? 'selected' : '' ?>>DESC
                            </option>
                            <option value="asc" <?= $sort == 'balance' && $order == 'asc' ? 'selected' : '' ?>>ASC
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-2 row custom-filter">
                    <label class="col-md-4 label2" for="">Available</label>
                    <div class="col-md-8">
                        <select class="form-control input-sm sorting" name="available">
                            <option value="">Sorting...</option>
                            <option value="desc" <?= $sort == 'available' && $order == 'desc' ? 'selected' : '' ?>>DESC
                            </option>
                            <option value="asc" <?= $sort == 'available' && $order == 'asc' ? 'selected' : '' ?>>ASC
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-2 row custom-filter">
                    <label class="col-md-4 label2" for="">Holding</label>
                    <div class="col-md-8">
                        <select class="form-control input-sm  sorting" name="holding">
                            <option value="">Sorting...</option>
                            <option value="desc" <?= $sort == 'holding' && $order == 'desc' ? 'selected' : '' ?>>DESC
                            </option>
                            <option value="asc" <?= $sort == 'holding' && $order == 'asc' ? 'selected' : '' ?>>ASC
                            </option>
                        </select>
                    </div>
                </div>
...
$(function() {
    ...
    $('.update-user-status').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "<?= base_url('/manager/advertiser/update_status') ?>",
            data: {
                id,
                status
            },
...
function sorting() {
    $('.sorting').change(function() {
        ...
        $.ajax({
            method: 'POST',
            url: `<?= base_url('manager/advertiser/list_account') ?>`,
            data: {
                sort: name,
                order: value,
            },
