...
class Advertiser extends CI_Controller
{
  ...
  function update_status_payment()
  {
    ...
    $this->db->trans_complete(); // Kết thúc transaction
  ...
  function list_products($page = 1)
  {
    ...
    if ($this->input->server('REQUEST_METHOD') === 'POST') {
      if ($this->input->post('reset_filter')) {
        $this->session->unset_userdata('product_filter');
      } else {
        $filter = $this->input->post();
        $this->session->set_userdata('product_filter', $filter);
      }
    }
    ...
    $networks = $this->Home_model->get_data('network', 'show = 1');
    ...
    $content = $this->load->view('advertiser_management/products.php', compact('products', 'offercats', 'networks', 'filter'), true);
    $this->load->view('admin/index', array('content' => $content));
  }
  ...
  private function build_where($filter)
  {
    ...
    if (!empty($filter['id'])) {
      $id = $this->db->escape($filter['id']);
      $where .= " AND cpalead_offer.id = {$id}";
    }

    if (!empty($filter['title'])) {
      $name = $this->db->escape_like_str($filter['title']);
      $where .= " AND cpalead_offer.title LIKE '%{$name}%'";
    }

    if (!empty($filter['offercats']) && is_array($filter['offercats'])) {
      $cats = $filter['offercats'];
      if (!empty($cats)) {
        $catString = 'o' . implode('o', $cats) . 'o';
        $where .= " AND offercat = " . $this->db->escape($catString);
      }
    }

    if (!empty($filter['network']) && is_array($filter['network'])) {
      $nets = array_map('intval', $filter['network']);
      $netStr = implode(',', $nets);
      $where .= " AND cpalead_offer.idnet IN ({$netStr})";
    }
    return $where;
  }
  ...
  public function add_new_advertiser()
  {
    ...
    return redirect(base_url(config_item('admin') . '/advertiser/list_account'));
  }
