...
<div class="row  mt-5">
    <div class="sc-dlyikq hrvHfq">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="col-auto">
                    <button type="submit" data-status="new_payment"
                        class="btn <?= $need_payment_class ?> mb-3 btn-sm edit_pp"
                        <?php echo $alert_string >= 50 ? '' : 'disabled'; ?>>Make Payment</button>
                </div>
        ...
        <div class="modal-content">
            <form class="formpayment" method="post" action="<?php echo base_url('v2/request_payouts'); ?>">
                ...
                <div class="modal-body">
                    <div class="row mb-2" id="amount">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                ...
                                <button type="button" class="btn btn-info btn-sm" id="chooseConversion"
                                    data-bs-toggle="modal" data-bs-target="#conversionModal">Choose Conversion</button>
                            </div>
                        </div>
                    </div>
                    <div class="row mb-2" id="amount">
                    ...
                    <div class="row mb-2">
                        <label for="payment_method" class="col-sm-4 col-form-label  col-form-label-sm">Payment
                            Method</label>
                        <div class="col-sm-8">
                    ...
                    <div id="BankWire" class="formpay">
                        ...
                        <div class="row mb-2 ">
                            ...
                            <div class="col-sm-8">
                                <span>174 Chau Thi Vinh Te, My An Ward, Ngu Hanh Son District, Da Nang City, Viet
                                    Nam</span>
                            </div>
                        </div>
                        <div class="row mb-3 ">
                    ...
                    <div class="row mb-2">
                        <label for="payment_image" class="col-sm-4 col-form-label col-form-label-sm">Proof
                            Payment</label>
                        <div class="col-sm-8">
                            <input type="file" name="payment_image" class="form-control form-control-sm"
                                id="payment_image" accept="image/*">
...
<div class="modal fade" id="conversionModal" tabindex="-1" aria-labelledby="conversionModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary btn-sm" id="transferToPayment" disabled>Transfer to
                    Payment</button>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function updateSelectedInfo() {
        ...
        $('.rowCheckbox:checked').each(function() {
            selectedCount++;
            const amount = parseFloat($(this).closest('tr').find('td:nth-child(12)').text().replace('$',
                '').trim()) || 0;
            ...
            success: function(response) {
                ...
                if (response && response.data && response.data.length > 0) {
                    response.data.forEach(item => {
                        ...
                        if (showCheckbox) {
                            content +=
                                `<td><input type="checkbox" class="rowCheckbox" value="${item.offerid}"></td>`;
                        }
                        showColumns.forEach(col => {
                            let value = item[col.key] !== undefined ? item[col
                                .key] : '';
                            if (col.key.startsWith('m')) value = '$' + parseFloat(
                                value).toFixed(2);
                            content += `<td>${value}</td>`;
                        });
                        content += `</tr>`;
    ...
    $('#transferToPayment').click(function() {
        ...
        updateOffersIdList();
        $('.rowCheckbox:checked').each(function() {
            const amount = parseFloat($(this).closest('tr').find('td:nth-child(12)').text()
                .replace('$', '').trim()) || 0;
            selectedTotal += amount;
        });
    ...
    $(document).on('click', '.btn-detail-payment', function() {
        ...
        loadConversionData({
            ...
            columns: ['offerid', 'oname', 'hosts', 'click', 'lead', 'approved', 'mapproved',
                'mtotal'
            ],
            showCheckbox: false
        });
