                  ...
                  <tbody>
                     ...
                     ?>
                           <tr class="<?php echo $clll; ?>">
                              ...
                              <td style="width:50px">
                              <?php if ($dulieu->flead != 1): ?>
                              <input
                                 name="amount2"
                                 class="amount2_input"
                                 style="width:50px"
                                 type="text"
                                 data-trackid="<?php echo $dulieu->id; ?>"
                                 data-offpercent="<?php echo (float)$dulieu->offerpercent; ?>"
                                 value="<?php echo (float)$dulieu->amount2 ?: ''; ?>"
                              >
                              <?php endif; ?>
                              </td>
                              ...
                              <?php if ($dulieu->flead != 1): ?>
                                 <input
                                    name="amount3_preview"
                                    class="amount3_preview"
                                    style="width:80px"
                                    type="text"
                                    readonly
                                    value=""
...
<script>
   ...
   function calcPointAdvFromPub(pub, offpercent) {
   var p = parseFloat(pub);
   var off = parseFloat(offpercent);

   if (isNaN(p) || p < 0) return '';
   if (isNaN(off)) off = 0;

   // FIX LOGIC: offerpercent = 0 ⇒ ADV = 0.00
   if (off <= 0) {
      return '0.00';
   }

   var pubShare = (100 - off) / 100; // off=40 => 0.6
   if (pubShare <= 0) return '0.00';

   var adv = p / pubShare;

   return (Math.round(adv * 1000) / 1000).toFixed(3);
   }


   function updateRowPointAdv($row) {
      var $pub = $row.find('input.amount2_input');
      if ($pub.length === 0) return;

      var pubVal = $pub.val();
      var offpercent = $pub.data('offpercent');

      var advVal = calcPointAdvFromPub(pubVal, offpercent);
      $row.find('input.amount3_preview').val(advVal);
   }

   // init
   $('#data_table tbody tr').each(function() {
      updateRowPointAdv($(this));
   });

   // realtime
   $(document).on('input', 'input.amount2_input', function() {
      updateRowPointAdv($(this).closest('tr'));
   });
