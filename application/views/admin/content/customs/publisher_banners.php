  ...
  <div class="row">
    <div class="box col-md-12">
        <div class="box-header">
            ...
            <div class="box-icon">
                <a class="btn-add" href="<?php echo base_url().$this->config->item('admin').'/route/banners/add/is_adv';?>"><i class="glyphicon glyphicon-plus"></i></a>
                <a class="btn-setting" href="#"><i class="glyphicon glyphicon-wrench"></i></a>
                <a class="btn-minimize" href="#"><i class="glyphicon glyphicon-chevron-up"></i></a>
                <a class="btn-close" href="<?php echo base_url().$this->config->item('admin').'/custom/view'?>"><i class="glyphicon glyphicon-remove"></i></a>
            </div>
        </div>
            ...
            <form>
                ...
                <?php foreach ($banners as $banner): ?>
                    <tr>
                        ...
                        <td>
                            <?php 
                            if($banner->show ==0){echo '<span data="id='.$banner->id.'&field=show&change=ShowHide" class="label label-warning ajaxst">Hide</span>';}
                            if($banner->show ==1){echo '<span data="id='.$banner->id.'&field=show&change=ShowHide" class="label label-success ajaxst">Show</span>';}                   
                            ?> 
                        </td>
                        <td>
                            <!--edit>>>-->
                            <a href="<?php echo base_url().$this->config->item('admin').'/custom/edit_publisher_banner/'.$banner->id . '/is_adv'?>" class="btn btn-info btn-xs">
                            <i class="glyphicon glyphicon-edit glyphicon glyphicon-white"></i>                                            
                            </a>
                            <!--delete>>>-->
                            <a href="<?php echo base_url().$this->config->item('admin').'/route/banners/delete/'.$banner->id;?>" class="btn btn-danger btn-xs">
