...
<section>
  ...
  <div class="col-12">
    <div class="box col-md-12">
      <form action="<?php echo base_url() . $this->config->item('admin') . '/custom/update/theme' ?>" method="POST">
        <div class="row">
...
<section>
  ...
  <div class="box-header">
    ...
    <div class="box-icon">
      <a class="btn-add" href="<?php echo base_url() . $this->config->item('admin') . '/custom/edit_register_page' ?>"><i class="glyphicon glyphicon-plus"></i></a>
      <a class="btn-close" href="<?php echo base_url() . $this->config->item('admin') . '/custom/' . $this->uri->segment(3); ?>"><i class="glyphicon glyphicon-remove"></i></a>
    </div>
  </div>
      ...
      <form>
        ...
        <?php foreach ($trafficTypes as $traffic) : ?>
          <tr>
            ...
            <td>
              <!--edit>>>-->
              <a href="<?php echo base_url() . $this->config->item('admin') . '/custom/edit_register_page/' . $traffic->id; ?>" class="btn btn-info btn-xs">
                <i class="glyphicon glyphicon-edit glyphicon glyphicon-white"></i>
              </a>
              <!--delete>>>-->
              <a href="<?php echo base_url() . $this->config->item('admin') . '/custom/delete/theme/' . $traffic->id; ?>" class="btn btn-danger btn-xs">
