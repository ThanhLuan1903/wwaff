...
$(document).ready(function () {
    ...
    function validateLanguageOnSubmit() {
        $('.percentage-error').remove();

        const isLanguageRequired = $('input[name="reqlang"]').val() === '1';

        if (!isLanguageRequired) {
            return true;
        }

        const checkboxLanguages = $('.langcontent input[type="checkbox"]:checked').length;
        const selectLanguages = $('select[name="lang[]"]').val();

        if (checkboxLanguages === 0 && (!selectLanguages || selectLanguages.length === 0)) {
            $('.langcontent').length ?
                $('.langcontent').after('<small class="text-danger percentage-error">Please select at least one language!</small>') :
                $('select[name="lang[]"]').after('<small class="text-danger percentage-error">Please select at least one language!</small>');
            return false;
        }
    ...
    $('select[name="lang[]"]').on('change', function () {
        $('select[name="lang[]"] + .percentage-error').remove();
    });
    /* ----------------------------------------------------------------- */
